...
When $y$ is an even number (when $y \text{ mod }  2 = 0$), we return $y + 1$ in the last expression in the code, so that's correct.
...
Taking the floor of $m + 1/2$ is just $m$ (remember that $m$ is an integer):
...
This is certainly a bit tricky at first, but provides an important lesson that induction is a solid way of proving correctness, even though most of it feels like magic.
