...
![Cover image](/blog/post-images/2024-03-17/19-lm-ch4.png)
...
![Stack for checking valid parentheses](/blog/post-images/2024-03-17/stack.gif)
...
In addition to `push` and `pop`, we'll also usually have a function like `peek` to get the topmost item in the stack, `is_empty` to check if the stack is empty, and `size` to get the size of the stack.
...
The first and only problem in this chapter is [Valid Parentheses](https://leetcode.com/problems/valid-parentheses), which we had a small peek into. Until then, happy coding.
